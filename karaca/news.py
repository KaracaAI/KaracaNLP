...
class News:
    ...
    @classmethod
    def guess(cls, text):
        model_dosya_adi = karaca_path + '/datas/newsDatas/newsModel.joblib'
        egitilmis_model = joblib.load(model_dosya_adi)

        vektorizer_dosya_adi = karaca_path + '/datas/newsDatas/newsVektorizer.joblib'
        yuklenmis_vektorizer = joblib.load(vektorizer_dosya_adi)

        label_encoder_dosya_adi = karaca_path + '/datas/newsDatas/newsLabelEncoder.joblib'
        ...
        return (f"Tahmin Edilen konu: {predicted_duygu[0]} % {predicted_probability:.2f}")
